...
## Unity / Agent
- `/qa` 스킬 §4 "기준별 Pass/Fail 증거 리포트" 가 RTM 의 실행 형태 — 기준 목록을 리포트 시점에 생성한다.
- 매핑 누수 스캔 스니펫 (필요할 때만):
  ```
  Grep "§[0-9]"    .claude/domain/*.md          # 기획 조항 ID 전수
  Grep "Spec: §"   Assets/Scripts/               # 매핑된 구현
  # diff → 매핑 안 된 조항
  ```
- 기획서(GDD)가 도착하면 `.claude/domain/gdd/` 하위 마스터 파일을 정하고, 조항 ID 는 그 파일 섹션 번호로 통일한다.
