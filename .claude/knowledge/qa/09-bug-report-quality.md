...
## Unity / Agent
- 재현 최소 단계는 **사용자가 Unity Editor에서 재현할 수 있는 단계**로 명시. "특정 씬 열고 → Play → 특정 동작 → 발생".
- Unity 로그 전문 붙이지 않기. `error CS` 코드·exception type·마지막 user code 프레임만.
- VR 버그는 재현 조건에 **플랫폼 명시** 필수: `Editor Play`, `Meta Quest 실기기`, `PICO 실기기` 중 어디서 발생하는지 — 환경마다 동작이 다를 수 있음 (§2).
