...
## Unity / Agent — 게임 맥락 (Real VR Fishing)
- 현재 프로젝트는 **한국어 우선**. 해외 출시 계획 생기면 이 섹션 전체 활성화.
- 정적으로 검증 가능한 것: `Grep "text\s*=\s*\"[가-힣]"` 로 하드코딩된 한국어 UI 문자열 스캔 → 로컬라이제이션 대상 목록.
- **TextMeshPro** 사용 여부 확인: `Grep "TextMeshProUGUI\|TMP_Text"` — 이미 TMP 쓰면 폰트 fallback 체인 확장만으로 다국어 대응 가능.
- 숫자 포맷은 `CultureInfo` 강제 (`value.ToString("N0", CultureInfo.CurrentCulture)`) — 하드코딩된 `","` 구분자 금지.
- VR 특이사항: Quest는 시스템 언어 설정을 따르므로 `OVRPlugin.GetSystemLocale()` 로 런타임 locale 감지 가능 — 해외 출시 시 진입점.
