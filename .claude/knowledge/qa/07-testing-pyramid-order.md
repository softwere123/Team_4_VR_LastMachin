...
## Unity / Agent
- 이 프로젝트의 기본 경로: **Unit·Integration 테스트 미존재 → 정적 분석(LSP/Grep) + 사용자 Play 확인**이 System 스모크 대체.
- 정적 분석으로 커버 가능한 것: RULES 위반, 이벤트 누수, Dave 경로 규칙, 재화 backing field 직접 쓰기.
- 정적 분석으로 커버 불가: VR 입력 반응, 햅틱 타이밍, 물리 시뮬레이션 체감 — 이 항목은 사용자에게 실기기 확인 요청이 유일한 방법임을 명시하고 보고.
- 실패 단계 요약: `정적 pass · 실기기 미검증` 형식으로 커버리지 한계를 명시.
