...
- 자주 쓰는 패턴:
  ...
  - `Assert.IsTrue(_isGrounded.HasValue, "State must be known before physics call")` — FixedUpdate 진입부 (RULE-03 방어).
  - `Assert.IsNotNull(ct, "CancellationToken must be provided")` — async 진입 (`rules/scripts.md` 컨벤션 방어).
